...
class S3Navigator:
    ...
    def __init__(self, profile: Optional[str] = None, region: str = "eu-central-1", serve: bool = False):
    ...
    def _handle_path_change(self, direction: str, item_name: Optional[str]) -> None:
        ...
        if direction == "up":
            ...
            item_idx = next((i for i, item in enumerate(self.current_items) 
    ...
    def _handle_item_selection(self, idx: int) -> None:
        ...
        if 0 <= idx < len(self.current_items):
            ...
            self.display.update_view(self.current_items, self.current_path, self.selected_items)
    ...
    def _list_objects(self) -> None:
        ...
        prefix = "/".join(self.current_path[1:]) + ("/" if len(self.current_path) > 1 else "")
